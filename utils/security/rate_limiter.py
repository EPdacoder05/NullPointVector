"""
Shared rate limiter for all guards.
"""
...
class RateLimiter:
    ...
    def is_rate_limited(self, key: str, guard_name: Optional[str] = None) -> Tuple[bool, Optional[float]]:
        ...
        # Get guard-specific limits if guard name is provided
        if guard_name:
            guard_config = security_config.get_guard_config(guard_name)
            requests_per_minute = guard_config.get(f'max_{guard_name.lower()}_per_minute', 60)
        else:
