"""
Shared input validation for all guards.
"""
...
class InputValidator:
    """Input validator for all guards."""
    
    def __init__(self):
        self.config = security_config.get_input_validation_config()
        
    def validate_message(self, message: str, guard_name: Optional[str] = None) -> Tuple[bool, Optional[str]]:
        """Validate a message.
        ...
        # Get guard-specific config if provided
        if guard_name:
            guard_config = security_config.get_guard_config(guard_name)
            max_length = guard_config.get('max_message_length', self.config.get('max_message_length', 10000))
        else:
