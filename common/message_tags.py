...
def categorize_message(*, subject: str = "", body: str = "", sender: str = "") -> List[Dict[str, str]]:
    ...
    if any(w in text for w in _SE) or len([c for c in tags if c["code"] in (
        ...
        add("SOCIAL_ENG", "Mood: social engineering")
    ...
    if spamish and not money and not pii and "JOB_SCAM" not in {t["code"] for t in tags}:
        add("SPAM_LIST", "Spam / marketing list")
    if money:
        add("MALICE_MONEY", "Malicious: money / payment ask")
    if any(w in text for w in _ADVANCE_FEE):
        add("ADVANCE_FEE", "Advance-fee / pay-to-unlock lure")
    if pii:
        add("MALICE_PII", "Malicious: PII / credential ask")
    if money or pii or "JOB_SCAM" in {t["code"] for t in tags} or "ADVANCE_FEE" in {t["code"] for t in tags}:
        add("THREAT_MALICE", "Threat class: malicious")
    elif spamish:
        add("THREAT_SPAM", "Threat class: spam (not cash/PII)")

    if any(w in text for w in ("security alert", "sign-in", "new login", "password reset")):
        add("SECURITY", "Security / account alert")
    ...
    if any(w in text for w in ("thank you for your interest", "unable to proceed")):
        add("REJECTION", "Application status")
    ...
    if any(t["code"] in (
        ...
        add("SENT_NEG", "Sentiment: negative / coercive")
    elif pos_hits > neg_hits and pos_hits > 0:
        add("SENT_POS", "Sentiment: positive / routine")
    elif not tags:
        add("SENT_UNK", "Sentiment: unclear")
    else:
        add("SENT_NEUT", "Sentiment: neutral")
